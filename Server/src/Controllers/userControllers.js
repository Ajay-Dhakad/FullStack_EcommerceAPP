...
const UpdateProfile = async (req, res) => {
    ...
    for(const key in req.body) {
        if (!req.body[key]) {
            return res.status(400).json({ success: false, message: `${key} cannot be empty or null` });
        }
        ...
        if (key === "name") {
            if (req.body[key].length < 3 || req.body[key].length > 16) {
                return res.status(400).json({ success: false, message: `${key} must be at least 3-16 characters long.` });
            }
        }
    }
    ...
    try {
        const updatedUser = await User.findOneAndUpdate({_id:req.user._id}, {...req.body},{new:true}).select('-password -carts -_id');
        ...
        return res.status(200).json({ success:true,message: 'Update successful',updatedUser });
    } catch (error) {
...
export {loginUser,signUpUser,getUser,UpdateProfile};